...
exports.decipherFormats = async (formats, html5player, options) => {
  ...
  for (const format of formats) {
    ...
    let url = format.url;
    try {
      if (format.sig && solvers.sig) {
        const deciphered = solvers.sig(format.sig);
        if (deciphered) url = url.replace(format.sig, deciphered);
      }
      if (format.nsig && solvers.nsig) {
        const deciphered = solvers.nsig(format.nsig);
        if (deciphered) url = url.replace(format.nsig, deciphered);
      }
    } catch {
